...
#'
#' @seealso [ngr_spk_rast_not_empty()], [fs::dir_ls()], [fs::file_delete()], [terra::rast()], [terra::values()]
#'
...
#' @export
#' @family spacehakr
ngr_spk_rast_rm_empty <- function(path = NULL, delete = TRUE, regexp = "\\.(tif|tiff|vrt)$", quiet = FALSE) {
  ...
  rast_files_with_data <- purrr::keep(rast_files, ngr_spk_rast_not_empty)
