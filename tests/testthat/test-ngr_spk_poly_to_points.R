...
test_that("ngr_spk_poly_to_points generates correct number of points within tolerance", {
...
test_that("ngr_spk_poly_to_points retains correct ID column name", {

  # Ensure the column name is "region"
  expect_true("region" %in% names(points), info = paste("Expected column 'region', found:", names(points)))
})

test_that("ngr_spk_poly_to_points assigns 'id' as default column name when col_id is NULL", {
  poly_no_id <- sf::st_sf(
  ...
  points <- ngr_spk_poly_to_points(poly_no_id, col_density = "col_density")
