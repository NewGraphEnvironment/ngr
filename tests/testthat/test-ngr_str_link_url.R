test_that("ngr_str_link_url generates a valid URL with correct anchor text", {
  repo_source <- "ngr"
  ...
  result_default <- ngr_str_link_url(repo_source = repo_source, url = url, target = target)
  ...
  result_repo <- ngr_str_link_url(repo_source = repo_source, url = url, anchor_text = repo_source, target = target)
  expect_equal(result_repo, expected_repo)
})

test_that("ngr_str_link_url generates a valid URL with no repo_source provided and default anchor text", {
