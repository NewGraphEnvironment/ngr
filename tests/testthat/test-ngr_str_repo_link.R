test_that("ngr_str_repo_link generates a valid URL with correct anchor text", {
  repo_source <- "ngr"
  ...
  result_default <- ngr_str_repo_link(repo_source = repo_source, url = url, target = target)
  ...
  result_repo <- ngr_str_repo_link(repo_source = repo_source, url = url, anchor_text = repo_source, target = target)
